...
PLACEHOLDER = re.compile(
    ...
    rb"from-file|secret-value|secret-token|"
    ...
    for line_number, line in enumerate(data.splitlines(), 1):
        ...
        for match in CREDENTIAL_ASSIGNMENT.finditer(line):
            ...
            if path.endswith(".py") and (
                separator == b":"
                or value.startswith((b"_", b"self.", b"os.", b"str(", b"dict(", b"getattr("))
                or b"(" in value
                or (not quote and re.fullmatch(rb"[A-Za-z_][A-Za-z0-9_.]*", value))
            ):
                continue
            rule = (
...
def scan_history(root: Path) -> list[Finding]:
    ...
    for line in _git(root, "rev-list", "--objects", "--all", "--reflog").splitlines():
    ...
    for check in checks:
        ...
        if kind != "blob" or int(raw_size) > MAX_BLOB_BYTES:
            continue
        findings.extend(scan_path(scope="history", identity=oid, path=objects[oid]))
        findings.extend(
            scan_data(
                scope="history",
                identity=oid,
...
def scan_commit_messages(root: Path) -> list[Finding]:
    findings: list[Finding] = []
    for raw_oid in _git(root, "rev-list", "--all", "--reflog").splitlines():
        oid = raw_oid.decode()
        commit = _git(root, "cat-file", "commit", oid)
