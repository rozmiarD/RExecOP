...
RExecOp `0.2.24a0` is the current source alpha line for operator evaluation.
...
| Source alpha line | `0.2.24a0` is current on `main`; see [CHANGELOG.md](../CHANGELOG.md) for history |
| Coordinated dependencies | Source line requires `govengine==0.16.11` and `sclite-core==1.0.9`; the `tecrax` extra requires `tecrax==0.3.21a0` |
