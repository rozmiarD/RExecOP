...
| Governed admission is source-qualified | GovEngine `typed_execution_governed_admission` v0.1/v0.2 consumption is qualified against the immutable `v1.0.0rc2` release-record commit `e65ad22ec25d74bbbb4969bd614981a8ed5e47c8`. This compatibility evidence does not configure host authority adapters or make mutation ready; configured mutation still fails closed without the complete governed-attempt boundary. |
| Plugin posture is a bounded projection | Explicit `fixture_only` and `operator_wrapper` selections bind neutral egress posture and control selection; they do not certify plugin code, wrapper implementation, entry-point provenance, process/network isolation, live infrastructure, mutation readiness, exactly-once I/O, or crash/power-loss recovery. |
